...
/*   Updated: 2024/10/28 11:34:21 by lmerveil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap() {
    std::cout << GREEN << "* [ClapTrap] Default constructor called here!" << std::endl << RESET;
}
...
void    ClapTrap::beRepaired(unsigned int amount) {
    ...
    if ((int)amount < 0)
            std::cout << RED << "[⁉️ invalid amount] Reperation amount needs to ne positive!" << RESET;
...
void    ClapTrap::takeDamage(unsigned int amount) {
    ...
    if (checkPowerLevel() == 0)
        return ;
    if ((int)amount < 0)
        std::cout << RED << "[⁉️ invalid amount] Damage amount needs to ne positive!" << RESET;
...
int     ClapTrap::checkPowerLevel() const {
    ...
    if (this->_energyPoints == 0 && this->_hitPoints == 0)
        ...
        return (std::cout << RED << "[🔋 OUT OF ENERGY] " << _Name << " has no more energy points to get repaired." << RESET << std::endl, 0);
    else if (this->_hitPoints == 0)
        return (std::cout << RED << "[💔 OUT OF HITS] " << _Name << " has no more hits points to attack." << RESET << std::endl, 0);
    else
        return (1);
}
